...
public class PurchaseService {
     ...
     * @return 进货记录列表
     */
    public PageDTO<PurchaseRecord> showPurRecordByPage(int page, int size) {
        //获取总记录条数
        int total = purchaseMapper.getCount();
        //根据page,size,total获取偏移量
        PageDTO<PurchaseRecord> pageDTO = new PageDTO<>(size, total, page);
        //获取记录集合，封装
        List<PurchaseRecord> allPurRecord = purchaseMapper.selectPurRecordByPage(pageDTO.getStart(), pageDTO.getPageSize());
        pageDTO.setData(allPurRecord);
        return pageDTO;
    }
